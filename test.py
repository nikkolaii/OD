...
def main(argv):
    ...
    #init.set_from_stream(str("10.22.67.168"),30000)
    init.set_from_stream(str("129.241.154.27"),30000)
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            for detection in detections:
             if detection[0] == "person":
                ...
                thickness = 1
                xd, yd, zd = get_object_depth(depth, bounds)
                if xd != -1 and yd != -1 and zd != -1: 
                 point = Point()
                 point.x = xd
                 point.y = yd
                 point.z = zd
...
if __name__ == "__main__":
    rospy.init_node("Camera_position_node", anonymous = False)
    pub = rospy.Publisher("person_position", Point, queue_size = 10)
